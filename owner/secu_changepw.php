...
<div id="main-containers">
   ...
   <?php
	...
	if(isset($_POST["submitbtn"]))
	{
		...
		$checkemail = mysqli_query($connect, "SELECT * from owner where owner_email='$email'");
		$check = mysqli_num_rows($checkemail);
		$email_check = mysqli_query($connect, "SELECT * from owner where owner_email= '$email' and secu_date >= '$currdate' and secu_pin = '$pinnum'");
		...
		if($check !=0)
		{
			if($count != 0)
			{
				if($pass1 == $pass2)
				{
					$update = mysqli_query($connect , "UPDATE owner set owner_pass= '$pass1' where owner_email= '$email'");
					if($update)
					{
						?> 
